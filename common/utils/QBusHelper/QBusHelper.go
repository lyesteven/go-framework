...
import (
	...
	topicConns "github.com/lyesteven/go-framework/common/utils/QBusHelper/proto"
...
var (
	...
	//chRequest   = make(chan reqCmd,1000)
	//chResponse  = make(chan struct {
	//	string
	//	error
	//},1000)
...
var ackHandler = func(ackedNuid string, err error) {
	if err != nil {
		log.Error("Warning: error publishing msg ","id", ackedNuid, "err",err.Error())
	} else {
		//log.Info("Received ack for msg", "id", ackedNuid)
	}
}
...
func publish(dcID ComMessage.CBSType, topic string, pubData []byte) error {
	...
	if pCon, ok := mapTopicConn[mapKey]; ok {
		if pCon.pubConn != nil && pCon.bCanUse {
			...
			if _,err := pCon.pubConn.nsConn.PublishAsync(topic, pubData,ackHandler); err == nil {
				// success
				log.Debug("published OK!", "topic:", topic, "  ClusterID---", pCon.pubConn.nsClusterID)
				return nil
			} else {       // else just for debug ############
				log.Error("       in publish() --7 fail")
			}
		}
	} else {
