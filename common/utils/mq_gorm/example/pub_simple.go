...
import (
	...
	"flag"
	"fmt"
	"github.com/lyesteven/go-framework/common/utils/mq_gorm"
...
func main() {
	...
	//初始化DB配置
	err := mq_gorm.InitWithMySQLCfg(ip, port, userName, password, dbName)
	if err != nil {
	...
	for {
		...
		for i := 0; i < 2; i++ {
			...
			data := []byte(msg)
			//发送事务性消息
			if err := mq_gorm.PublishTransMsg(topic, data, &Callable{}); err != nil {
