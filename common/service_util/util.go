...
import (
	"fmt"
	"gworld/git/third-package/satori/go.uuid"
...
func generateVersion() chan string {
	...
	go func() {
		for {
			u1, err := uuid.NewV4() //对uuid的唯一性还是有问题的，这个库只是一个随机数，用在这里显然不合适
			if err != nil {
				log.Error("generateVersion", "uuid.NewV4", err)
				continue //if err every time, dead loop
			}
