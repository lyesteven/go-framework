...
func main() {
	...
	i = 0
	serviceUtil := su.NewServiceUtilForSearch()
	...
	if startSeq == 0 {
		...
		qbh.SubTopicFromDC(ComMessage.CBSType_Base,topic, "gp-1", "abc", 50, Msgcb)
	} else {
		//qbh.SubTopicAt(topic, "gp-1", "abc", 1, startSeq, Msgcb)
		qbh.SubTopicAtFromDC(ComMessage.CBSType_Base,topic, "gp-1", "abc", 1, startSeq, Msgcb)
	}
